...
#diversity in relation to odi  in participants with valid T90/odi
  ...
  p2=ggplot(dat.plot, aes(x=Axis.1,y=Axis.2, color=odicat))+
    # geom_point(size=1.1) +
  ...
  p2$labels$colour <- "ODI severity groups"
  
  p2 <- p2 + theme(legend.title = element_blank()) 
  
  p2 <- p2 + guides(color=guide_legend(nrow=2))
  
  ggsave("BC.odicat.png", plot = p2, device = "png", 
         path = "/home/baldanzi/Sleep_apnea/Results/")
  
  ggsave("BC.odicat.pdf", plot = p2, device = "pdf", 
