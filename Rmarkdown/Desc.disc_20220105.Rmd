...
```{r, }
perma.res = lapply(perma.res,function(dd){
  apply(dd,2,function(x){ifelse(!is.na(x),x,"")})
  })
...
knitr::kable(list.res$table, 
               caption = "Pairwise comparisons between T90 groups") %>% 
    kable_styling()

pheno <- readRDS("/home/baldanzi/Datasets/sleep_SCAPIS/pheno.MGS.Upp.rds")
pheno[, t90cat:=factor(t90cat, levels(t90cat), 
                         labels = c("t0", "t1", "t2", "t3"))]
t = pheno %>% filter(valid.t90=="yes") %>% group_by(t90cat) %>% count() 

knitr::kable(t, 
               caption = "Size of T90 groups") %>% 
    kable_styling()
...
```{r, warning=FALSE, message=FALSE}
sig.mgs <- unique(do.call(c,mgs.m2)) 

  t2 <-  data.table(MGS = sig.mgs, 
                    prevalence = apply(decostand(pheno[,sig.mgs,with=F], "pa"), 2, sum)/nrow(pheno))
  

  t2[,prevalence:= round(prevalence*100,1)]

 t3 <- t[exposure!="BMI" & MGS %in% sig.mgs, .(MGS,exposure,rho,q.value)]
 setnames(t3,"q.value","q")
 t3 <- dcast(t3, MGS ~ exposure, value.var=c("rho","q"))
 
 t2 <- merge(t2,t3,by="MGS")
...
modules.ahi = t[sig_ahi==T & sig_BMI==F, pathway]
modules.t90 = t[sig_t90==T & sig_BMI==F, pathway]
modules.odi = t[sig_odi==T & sig_BMI==F, pathway]
...
```{r, out.width='100%', out.height='100%', fig.align='center', fig.cap="Subpathways enriched among correlation between MGS and metabolites"}
knitr::include_graphics("/proj/nobackup/sens2019512/wharf/baldanzi/baldanzi-sens2019512/mgs_subpathway_ea_heatmap_gutsy.pdf")
```
