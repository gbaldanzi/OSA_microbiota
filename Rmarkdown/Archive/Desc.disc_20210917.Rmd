...
Self-reported sleep apnea by OSA severity based on AHI from Apnealink 

```{r , include=FALSE, message=FALSE, echo=FALSE, warning=FALSE}
t.ahi.self <- compareGroups(OSAcat ~ cqhe058, data=pheno[valid.ahi=='yes',]) 
export2md(createTable(t.ahi.self), which.table = "descr", format = "html", caption="Self-reported sleep apnea across the OSA severity groups based on measured AHI from Apnealink") %>%  kable_styling()
```
...
```{r,include=FALSE, echo=FALSE, message=FALSE, warning=FALSE}
  res.m2 <-  fread("/home/baldanzi/Sleep_apnea/Results/cor2_all.var_mgs.tsv")
    ...
    cor_mgs.fdr <- cor_mgs.fdr %>% pivot_wider(id_cols=MGS, names_from = exposure, values_from = c(cor.coefficient,p.value,q.value))
    setDT(cor_mgs.fdr)
...
## Scatter plot of correlation coefficients {.tabset}
    
&nbsp;

Scatter plot of correlation coefficients from 2 analysis (correlation between AHI or T90 and MGS & correlation between Self-reported apnea and MGS)

```{r, results='asis', echo=FALSE, fig.show='hold', out.width='50%'}
knitr::include_graphics('/home/baldanzi/Sleep_apnea/Results/Plots/scatter_self_ahi.png')
knitr::include_graphics('/home/baldanzi/Sleep_apnea/Results/Plots/scatter_self_t90.png')
```
...
Plots display the MGS prevalence in all SCAPIS participants (n = `r n2`) by self-reported sleep apnea
...
## BC by CPAP ttment among participants with self-reported apnea
...
## BC by CPAP ttment among participants with severe apnea, measured AHI
...
## BC by CPAP ttment among participants with moderate/severe apnea, measured AHI
...
```{r, message=FALSE, warning=FALSE,echo=FALSE,fig.align='center'}
  ...
  all.data=fread("/home/baldanzi/Datasets/sleep_SCAPIS/SCAPIS-DATA-PETITION-170-20210315.csv", header = T, na.strings=c("", "NA"))
  setnames(all.data, "Subject", "SCAPISid") #renaming the id variable
  ...
  all.data <- merge(all.data, data.MGS, by="SCAPISid", all.y=T)

  all.data[,self.apnea:=cqhe058]
  all.data[self.apnea=="NO_ANSWER", self.apnea:=NA]
  all.data[,CPAP:='NO']
  all.data[cqhe061=='CPAP',CPAP:='YES']
  
  t <- compareGroups(Site~CPAP+self.apnea,data=all.data)
  export2md(createTable(t), which.table = "descr", format = "html", caption="Self-reported CPAP use by center") %>% kable_styling()
```
...
```{r, message=FALSE, warning=FALSE,echo=FALSE,fig.align='center'}
  ...
  all.data[SCAPISid %in% sleep$SCAPISid, sleep.study:='yes']
  all.data[!SCAPISid %in% sleep$SCAPISid, sleep.study:='no']
  
  t <- compareGroups(sleep.study~CPAP+self.apnea,data=all.data)
  export2md(createTable(t), which.table = "descr", format = "html", caption="Self-reported CPAP use by inclusion to the sleep recording") %>%  kable_styling()
``` 
...
```{r, message=FALSE, warning=FALSE,echo=FALSE,fig.align='center'}
  t <- compareGroups(sleep.study~CPAP+self.apnea,data=all.data[Site=="Site5",])
  export2md(createTable(t), which.table = "descr", format = "html", caption="Self-reported CPAP use by inclusion to the sleep recording - Uppsala only") %>%   kable_styling()
``` 
...
```{r, message=FALSE, warning=FALSE,echo=FALSE,fig.align='center'}
  ...
  all.data[SCAPISid %in% sleep[valid.ahi=='yes',SCAPISid], sleep.study.valid:='yes']
  all.data[!SCAPISid %in% sleep[valid.ahi=='yes',SCAPISid], sleep.study.valid:='no']
  
  t <- compareGroups(sleep.study.valid~CPAP+self.apnea,data=all.data)
  export2md(createTable(t), which.table = "descr", format = "html", caption="Self-reported CPAP use by valid measurement in the sleep recording") %>%  kable_styling()
``` 
...
```{r, message=FALSE, warning=FALSE,echo=FALSE,fig.align='center'}
  ...
  t <- compareGroups(sleep.study.valid~CPAP+self.apnea,data=all.data[Site=="Site5",])
  export2md(createTable(t), which.table = "descr", format = "html", caption="Self-reported CPAP use by valid measurement in the sleep recording - Uppsala only") %>%  kable_styling()
``` 
...
```{r, message=FALSE, warning=FALSE,echo=FALSE,fig.align='center'}
  ...
  all.data[SCAPISid %in% sleep[cpap=='yes',SCAPISid], cpap.study.night:='yes']
  all.data[!SCAPISid %in% sleep[cpap=='yes',SCAPISid], cpap.study.night:='no']
  
  t <- compareGroups(sleep.study.valid~CPAP+self.apnea+cpap.study.night,data=all.data[SCAPISid %in% sleep$SCAPISid,])
  export2md(createTable(t), which.table = "descr", format = "html", caption="Self-reported CPAP use by valid measurement only in those included in the sleep study") %>%  kable_styling()
``` 
...
```{r, message=FALSE, warning=FALSE,echo=FALSE,fig.align='center'}
  ...
  t <- compareGroups(CPAP~cpap.study.night+self.apnea,data=all.data[SCAPISid %in% sleep$SCAPISid,])
  export2md(createTable(t), which.table = "descr", format = "html", caption="Use of CPAP during the study night by self-reported CPAP use in the questionnaire - only sleep study participants") %>%  kable_styling()
``` 
