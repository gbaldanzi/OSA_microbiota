...
correlation = round(with(valid.ahi[!is.na(sat90),], cor(ahi,sat90, use="complete.obs")),3)

correlation2 = round(with(valid.ahi[!is.na(odi),], cor(ahi,odi)),3)

correlation3 = round(with(valid.t90[!is.na(odi),], cor(sat90,odi)),3)
```
...
* Correlation between AHI and T90 = **`r correlation`**   
* Correlation between AHI and ODI = **`r correlation2`**
* Correlation between ODI and T90 = **`r correlation3`** 
<br>
...
- Measurement of intermittent hypoxia
...
## ODI 
_Oxygen desaturation index_     

Participants with valid ODI
N = **`r n1`** 
     
```{r odi_histogram, echo=FALSE, fig.align='center', out.width="50%", out.height="70%"}
knitr::include_graphics("/home/baldanzi/Sleep_apnea/Descriptive/hist.odi.png")
```
...
```{r,echo=FALSE, message=FALSE, warning=FALSE}
t1 <- readRDS('/home/baldanzi/Sleep_apnea/Descriptive/sleepapnea_ahi_table1.rds')
...
t1 <-readRDS('/home/baldanzi/Sleep_apnea/Descriptive/sleepapnea_t90_table1_.rds')
...
diff2=abs(valid.ahi[,Date]-as.Date(valid.ahi[,AnthropometryCollectionDate]))
...
knitr::include_graphics("/home/baldanzi/Sleep_apnea/Descriptive/hist.shannon.png")
```
<br>
<br>
...
#### {.unlisted .unnumbered}
1. Basic model= sex + age + smoking + alcohol + plate 
2. Full model = BMI + PA + education + place birth + fiber intake + energy intake + visit month + medication
...
#### Spearman Correlation between T90% and Shannon
...
#### Spearman Correlation between ODI and Shannon

   
```{r corrshannonODI, echo=FALSE, message=FALSE, warning=FALSE,fig.align='center'}
shannoncor = fread("/home/baldanzi/Sleep_apnea/Results/cor_odi_alpha.tsv", header=T)
temp = c(6,7)
shannoncor = shannoncor[,-temp, with=F]
setnames(shannoncor, "MGS", "Var 1")
setnames(shannoncor, "exposure", "Var 2")
...
shannoncor[,p.value:=NULL]
setcolorder(shannoncor,c("model"))
knitr::kable(shannoncor, caption = "Correlation between ODI and Shannon diversity index", 
             align = c('l','c','c','c','c','c')) %>% 
  kable_styling()
```
...
n3 = perma_t90_BC3[variables =="t90cat", 'Pr(>F)']
```
