...
res.pca <- fread("/home/baldanzi/Sleep_apnea/Results/cor_pc.tsv")
res.pca[,rho:=round(rho,3)]
res.pca[,p.value:=round(p.value,5)]
res.pca[,q.value:=round(q.value,5)]

res.ahi <- res.pca[exposure=="ahi",.(outcome, model, rho, p.value, q.value, N)]
res.t90 <- res.pca[exposure=="t90",.(outcome, model, rho, p.value, q.value, N)]

outcomes <- unique(res.pca[,outcome])[order(unique(res.pca[,outcome]))]
```
...
for(m in outcomes){
  ...
  cat('  \n')
  cat('\n **AHI** \n')
  cat(kable(res.ahi[outcome==m,-("outcome")],format = "html", table.attr = "style='width:50%;'") %>% kable_styling(position = "center"))
  cat('\n **T90** \n')
  cat(kable(res.t90[outcome==m,-("outcome")],format = "html", table.attr = "style='width:50%;'") %>% kable_styling(position = "center"))
  cat('  \n')
}
```
&nbsp;
...
t.ahi <- t[MGS %in% mgs.fdr.m2$mgs.fdr.ahi,.(MGS,cor_m2ahi,p.value_m2ahi, cor_saahi,p.value_saahi,q.value_saahi)]

t.t90 <- t[MGS %in% mgs.fdr.m2$mgs.fdr.t90,.(MGS,cor_m2t90,p.value_m2t90, cor_sat90,p.value_sat90,q.value_sat90)]
...
sketch2 = htmltools::withTags(table(
  ...
  thead(
    ...
    tr(
      lapply(rep(c(' rho ', ' p-value ', ' rho ', ' p-value ','q-value'), 1), th)
    )
  )
))
