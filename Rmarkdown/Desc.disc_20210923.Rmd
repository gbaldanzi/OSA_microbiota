...
- *Data on MGS-metabolite correlations was available for 23*
...
```{r,include=FALSE, echo=FALSE, message=FALSE, warning=FALSE}
  ...
  mgs.fdr.m2 <- readRDS('/home/baldanzi/Sleep_apnea/Results/mgs.m2.rds')
  mgs.fdr.ahi <- mgs.fdr.m2$mgs.fdr.ahi
  mgs.fdr.t90 <- mgs.fdr.m2$mgs.fdr.t90
    
    # Results from the correlations with self reported sleep apnea 
    ...
    # Determine direction of correlation between MGS and measured AHI
    mgs.pos <- res.m2[MGS %in% mgs.fdr.ahi & exposure %in% c("ahi") & cor.coefficient>0,MGS]
    mgs.neg <- res.m2[MGS %in% mgs.fdr.ahi & exposure %in% c("ahi") & cor.coefficient<0,MGS]
    
    t <- res.self[MGS %in% mgs.fdr.ahi,.(MGS,cor,pval)]
    t <- t[MGS %in% mgs.pos, cor_direction:='pos']
    t.ahi <- t[MGS %in% mgs.neg, cor_direction:='neg']
    
        # Determine direction of correlation between MGS and measured T90 
    mgs.pos <- res.m2[MGS %in% mgs.fdr.t90 & exposure %in% c("t90") & cor.coefficient>0,MGS]
    mgs.neg <- res.m2[MGS %in% mgs.fdr.t90 & exposure %in% c("t90") & cor.coefficient<0,MGS]
    
    t <- res.self[MGS %in% mgs.fdr.t90,.(MGS,cor,pval)]
    t <- t[MGS %in% mgs.pos, cor_direction:='pos']
    t.t90 <- t[MGS %in% mgs.neg, cor_direction:='neg']
```    
...
```{r,}    
    ...
    datatable(t.ahi,rownames = FALSE,  
              options = list(
...
```{r,}    
    ...
    datatable(t.t90,rownames = FALSE,  
              options = list(
            columnDefs = list(list(className = 'dt-center', targets = c(2:3)) )
            )) %>% formatStyle(c(3), `border-right` = "solid 2px")
