...
t <- cor(pheno[,.(ahi,t90,odi,BMI)], use="complete.obs",method = "spearman")
t <- round(t,2)

kable(t, format = "html", table.attr = "style='width:40%;'",
      caption = "Correlation between OSA parameters and BMI") %>% 
...
main model no BMI = age + sex + smoking + alcohol + extraction batch + shannon
...
res.BMI <- fread(paste0(results.folder,"cor.bmi_all.var_mgs_temp.tsv"))
...
These species will be analyzed with the extended model. 

<br>


Number of species with a q-value<0.05 for each OSA variable in the main model without BMI adjustment
...
## Main model with BMI {.tabset .tabset-pills}

<br>

<div class = "blue">

Covariates = age + sex + smoking + alkohol + shannon + plate + BMI 
...
**`r n`** species associated with at least one of the three OSA variables in the main model with BMI adjustment

</div>

<br>

Number of species with a q-value<.05 for each OSA variable in the main model with BMI 
...
**`r n`** species associated with at least one of the three OSA variables in the extended model.     

</div>

<br>

Number of species with a q-value<.05 for each OSA variable in the extended model
...
res.sa.med <- fread("/home/baldanzi/Sleep_apnea/Results/corsamed_all.var_mgs.tsv")
res.fm <- fread("/home/baldanzi/Sleep_apnea/Results/cor2_all.var_mgs.tsv")
...
res.fm <- res.fm[,a,with=F]
res.fm[,rho:=round(rho,3)]
res.fm[,p.value:=round.large(p.value)]


res.fm <- res.fm[exposure %in% c("odi","t90"),names(res.sa.med),with=F]
names(res.fm)[-c(1:2)] <- paste0(names(res.fm)[-c(1:2)], "_full_model")
names(res.sa.med)[-c(1:2)] <- paste0(names(res.sa.med)[-c(1:2)], "_SA")

res.sa.med <- merge(res.sa.med, res.fm, by=c("MGS","exposure"), all.x=T, all.y=F)
...
sketch2 = htmltools::withTags(table(
  ...
  thead(
    tr(
      th(rowspan = 2, 'MGS'),
      th(colspan = 3, 'Full Model'),
      th(colspan = 4, 'SA Medication'),
    ),
    tr(
...
Re-run the full model analysis for the species identified with the full model  after excluding participants that had used any antibiotic in the last 6 months 
...
res.fm <- res.fm[exposure %in% c("odi","t90"),names(res.sa.atb),with=F]
names(res.fm)[-c(1:2)] <- paste0(names(res.fm)[-c(1:2)], "_full_model")
names(res.sa.atb)[-c(1:2)] <- paste0(names(res.sa.atb)[-c(1:2)], "_SA")

res.sa.atb <- merge(res.sa.atb, res.fm, by=c("MGS","exposure"), all.x=T, all.y=F)

t.t90 <- res.sa.atb[exposure=="t90",]

t.t90 <- t.t90[MGS %in% mgs.fdr$t90,.(MGS,rho_full_model,p.value_full_model,N_full_model, rho_SA,p.value_SA,q.value_SA,N_SA)]

t.odi <- res.sa.atb[exposure=="odi"]

t.odi <- t.odi[MGS %in% mgs.fdr$odi,.(MGS,rho_full_model,p.value_full_model,N_full_model, rho_SA,p.value_SA,q.value_SA,N_SA)]


table.fun <- function(x) {datatable(x, container=sketch2,rownames = FALSE, class = 'cell-border stripe', options = list(
  rowCallback = JS(
    "function(row, data) {",
    "for (i = 1; i < data.length; i++) {",
    "if ( data[i]<0.001 & data[i]>0){",
