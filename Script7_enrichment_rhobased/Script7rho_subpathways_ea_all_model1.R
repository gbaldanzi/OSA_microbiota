...
# Rho based
  ...
  list_mm_correlation.sp <-  readRDS('/home/baldanzi/Sleep_apnea/Results/list_mm_correlation.sp.rds')
  
  set.seed(123)
  res.ahi <- fgsea(pathways = subpathways, stats = rank(list_mm_correlation.sp[[1]]), eps=0,scoreType="pos", nPermSimple = 100000, nproc=3)
  res.ahi=cbind("AHI",res.ahi)
  saveRDS(res.ahi,'/home/baldanzi/Sleep_apnea/Results/temp_subpath_ea_ahi.rds')
  
  res.bmi <- fgsea(pathways = subpathways, stats = rank(list_mm_correlation.sp[[2]]), eps=0,scoreType="pos", nPermSimple = 100000)
  res.bmi=cbind("BMI",res.bmi)
  saveRDS(res.bmi,'/home/baldanzi/Sleep_apnea/Results/temp_subpath_ea_bmi.rds')
  
  res.t90 <- fgsea(pathways = subpathways, stats = rank(list_mm_correlation.sp[[3]]), eps=0,scoreType="pos", nPermSimple = 100000)
  ...
  fwrite(res.subpath.ea, '/home/baldanzi/Sleep_apnea/Results/ea_subpath_rho.tsv',sep='\t')