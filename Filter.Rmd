...
noms=grep("____",names(dades.real),value=T)
...
data_pa <- decostand(x = dades.real[,noms], "pa")
...
# Data set with the MGS divided by how prevalent they are
  t = sapply(cut.offs, function(x) {nrow(data_sum[data_sum$prevalence>=x,])})
  t = data.frame(Present=t)
  t$Threshold = factor(paste0(cut.offs," indiv."),
                       levels = paste0(cut.offs," indiv."))
