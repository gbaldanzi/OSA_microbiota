...
# This code will create a table with the results for the SA2 for the relevant MGS
  ...
  # Importing results 
  res.sa3ahi <- fread("/home/baldanzi/Sleep_apnea/Results/corSA3_ahi_mgs.tsv")
  res.sa3t90 <- fread("/home/baldanzi/Sleep_apnea/Results/corSA3_t90_mgs.tsv")
  res.m2 <-  fread("/home/baldanzi/Sleep_apnea/Results/cor2_all.var_mgs.tsv")
  
  res.sa3ahi[q.value>=0.001, q.value:=round(q.value, digits = 3)]
  res.sa3t90[q.value>=0.001, q.value:=round(q.value, digits = 3)]
  ...
  res.sa3ahi <- res.sa3ahi[MGS %in% mgs.fdr, a, with = F]
  res.sa3t90 <- res.sa3t90[MGS %in% mgs.fdr, a, with = F]
  res.m2ahi <- res.m2[MGS %in% mgs.fdr & exposure == "ahi", a, with = F]
  res.m2t90 <- res.m2[MGS %in% mgs.fdr & exposure == "t90", a, with = F]
  
  res.list <- list(m2ahi= res.m2ahi, sa3ahi=res.sa3ahi,
                   m2t90=res.m2t90, sa3t90=res.sa3t90)
  ...
  saveRDS(res.table,file="/home/baldanzi/Sleep_apnea/Results/table.res_sa3.rds")
