...
# Assign object with BMI groups and models
  group <-  c("<25","25-30",">=30")
  mo <- unique(cor.t90[,model])

# Create plots and tables for every BMIcat

for(i in 1:length(group)){
  cor.t90 <- fread(paste0(input,"cor_t90_mgs_bmi",group[i],".tsv"), sep=",")
  setnames(cor.t90,'cor.coeficient','cor')
  for(j in 1:length(mo)){
    a = sum(cor.t90[model==mo[j],q.value]<.05,na.rm=T)
    n = nrow(cor.t90[model==mo[j], ])
    
    p = ggplot(cor.t90[model==mo[j],],aes(y=-log(q.value),x=phylum,color=phylum))+
    ...
    ggsave(p, filename = paste0("res.plot_t90_",i,"_",mo[j],".png"), device = "png",
           path = output.plot)
    
    t = cor.t90[model==mo[j],] %>% select(MGS, exposure, cor, p.value,q.value,N) %>% arrange(q.value)
    ...
    saveRDS(t, file = paste0(output.plot,"res.table_t90_",group[i],"_",mo[j],".rds"))
