...
# This code aims to find discordant MGS associated with either BMI or T90%


# Results saved at "/home/baldanzi/Sleep_apnea/Results/discordant_mgs_t90.tsv"
...
# Correlation between t90 and MGS NOT ADJUSTING FOR BMI ####
...
t1 = Sys.time()
print("Spearman correlations for t90 models 1-3 - time")
print(t1-t0)
...
fwrite(step1.res, file = paste0(output,"discordant_t90.tsv"), sep="\t")

#--------------------------------------------------------------------------#

# Correlation between t90 and BMI with same models ####
...
fwrite(step1.res, file = paste0(output,"discordant_bmi_t90.tsv"), sep="\t")
...
dades <- fread(paste0(output,"discordant_t90.tsv"))
dades <- merge(dades, taxonomy, by="MGS", all.x=T)
fwrite(dades, file=paste0(output,"discordant_t90.tsv"))

dades <- fread(paste0(output,"discordant_bmi_t90.tsv"))
dades <- merge(dades, taxonomy, by="MGS", all.x=T)
fwrite(dades, file=paste0(output,"discordant_bmi_t90.tsv"))