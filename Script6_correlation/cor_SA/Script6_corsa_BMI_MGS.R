...
noms = grep("____",names(pheno),value=T) # vector with MGS names 
pheno[,shannon:=diversity(pheno[,noms, with=F],index="shannon")]
...
dades <- fread(paste0(output,"corsa_BMI_mgs.tsv"))
dades <- merge(dades, taxonomy, by="MGS", all.x=T)
fwrite(dades, file=paste0(output,"corsa_BMI_mgs.tsv"))