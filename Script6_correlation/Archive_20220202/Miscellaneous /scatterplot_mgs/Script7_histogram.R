...
# This script will produce histogram for MGSs correlated to 
# T90 and AHI in the fully adjusted model 
...
# Select the relevant MGSs for t90
mgs.bmi <- res.bmi[q.value<.05,]
mgs.t90 <- res.t90[q.value<.05 & !MGS %in% mgs.bmi$MGS,MGS]

# Select the relevant MGSs for AHI
mgs.bmi <- res.bmi[q.value<.05,]
mgs.ahi <- res.ahi[q.value<.05 & !MGS %in% mgs.bmi$MGS,MGS]
mgs.ahi <- mgs.ahi[!mgs.ahi %in%  mgs.t90]

    mgs <- c(mgs.ahi,mgs.t90)
    mgs <- mgs[order(mgs)]
    
    
# Histogram ####

plot_histo = function (var.x,data) {
...
# Plot
    h.ahi <- plot_histo("ahi", data =valid.ahi)
    h.t90 <- plot_histo("t90", data =valid.t90)
  
    hist.mgs <- lapply(mgs, plot_histo, data = valid.t90)
    

pdf(paste0(output.plot,"plot_histo_mgs_model2.pdf"))
ggarrange(plotlist = c(list(h.ahi,h.t90),hist.mgs), ncol = 2, nrow = 2)
dev.off()
