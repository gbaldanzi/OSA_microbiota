...
# correlated to AHI, T90 and BMI
  ...
  res.pos <-  lapply(res.pos,function(x){x[order(q.value) & q.value<0.05,.(family,pval,q.value)]})
  res.neg <-  lapply(res.neg,function(x){x[order(q.value) & q.value<0.05,.(family,pval,q.value)]})
  
  #Saving 
  saveRDS(res.both,file=paste0(output,"ea_family_table_both.rds"))
  saveRDS(res.pos,file=paste0(output,"ea_family_table_pos.rds"))
  saveRDS(res.neg,file=paste0(output,"ea_family_table_neg.rds"))