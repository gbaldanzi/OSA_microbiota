...
# Function to select the last characters of a string 
 ...
 # Enriched pathways/metabolite groups in the NEGATIVE correlations ####
  ...
  ha1n =  HeatmapAnnotation(foo = anno_block(gp = gpar(fill = "white", col = 0),
                                            labels = c("negative", "positive"),
                                            labels_gp = gpar(col = "black", fontsize = 8)))
  
  ha2n =  HeatmapAnnotation(foo = anno_block(gp = gpar(fill = "white", col = 0),
  ...
  h2 = Heatmap(t90.odi.cor2, 
               cell_fun = function(j, i, x, y, w, h, fill) {
                 if(t90.odi.q2[i, j] < 0.05) {
                   grid.text('*', x, y)
                 } },
               column_names_rot=40, 
              ...
              # heatmap_legend_param = list( title = expression(rho),
               ...
               show_row_dend = F, 
               width = unit(4*ncol(t90.odi.cor2),"mm")) +
    
        Heatmap(merged.matrix2, 
             cell_fun = function(j, i, x, y, w, h, fill) {
               if(merged.matrix.q2[i, j] < 0.05) {
                 grid.text('*', x, y)
               } },
             column_names_rot =  40, 
             column_names_side = "bottom",
             ...
             width = unit(4*ncol(merged.matrix2),"mm"), 
             height = unit(5*nrow(merged.matrix2),"mm"))  + 
  ...
  # Draw ####
  pdf(file = paste0(results.folder, "mgs_subpathway_ea_heatmap_gutsy_neg3.pdf"), 
      width = 9, height = 9)
  set.seed(7)
  draw(h2, heatmap_legend_side = "right", show_heatmap_legend=F, ht_gap = unit(c(3, 6), "mm"))
