...
# The venn2 include the analysis from step 2
  ...
  # Importing results 
  res <- fread(paste0(input,"cor_sa_atb6m_step2_all.var_mgs.tsv"))
  ...
  venn <- ggvenn(mgs.fdr,
                    ...
                    show_percentage = F, 
                    fill_alpha = .6) +
    ggtitle("MGS correlated with AHI, T90%, and BMI\nexcluded antibiotic use last 6m\nFully adjusted model") +
    theme(plot.title = element_text(size=14, face = "bold", hjust = 0.5))
  
  ggsave("Venn_bmiahi_sa_atb2.png",plot = venn,device = "png", path=output.plot)


  ggsave("Venn_bmiahi_sa_atb2.png",plot = venn,device = "png", 
