...
# Function to select the last characters of a string 
 ...
 # Enriched pathways/metabolite groups in the NEGATIVE correlations ####
  ...
  # Association with SBP/DBP/HbA1c ####
    ...
    res.mgs.bp.cor <- res.mgs.bp[MGS_features %in% pos.mgs,.(MGS_features,outcomes,rho)] %>% 
      spread(key=outcomes, value = rho) # add osa.gmm to MGS features
    res.mgs.bp.q <- res.mgs.bp[MGS_features %in% pos.mgs,.(MGS_features,outcomes,q.value)] %>% 
  ...
  h1 = Heatmap(t90.odi.cor1, 
               cell_fun = function(j, i, x, y, w, h, fill) {
                 if(t90.odi.q1[i, j] < 0.05) {
                   grid.text('*', x, y)
                 } },
               column_names_rot=40, 
              ...
              # heatmap_legend_param = list( title = expression(rho),
               ...
               show_row_dend = F, 
               width = unit(4*ncol(t90.odi.cor1),"mm")) +
    
        Heatmap(merged.matrix1, 
             cell_fun = function(j, i, x, y, w, h, fill) {
...
#  dev.off()
  ...
  # Draw ####
  pdf(file = paste0(results.folder, "mgs_subpathway_ea_heatmap_gutsy_draft.pdf"), 
      width = 11, height = 9)
  set.seed(7)
