...
```{r DAG, echo=FALSE, out.width="80%", out.height="80%",fig.align='center'}
...
```{r, echo=FALSE, fig.align='center', fig.cap="Scatter of AHI and Shannon index"}
...
```{r, echo=FALSE, fig.align='center', fig.cap="Scatter of T90 and Shannon index"}
...
perma.res.osa = apply(perma.res.osa,2,function(x){ifelse(!is.na(x),x,"")})

for(m in mod.names){
  cat(sprintf("### %s {-}", m))
  knitr::kable(perma_OSA_BC1, 
...
<br>
```{r,}
kable(ps,format = "html", table.attr = "style='width:30%;'", 
        caption = 'PERMANOVA test - p.value') %>%
  #kable_classic(full_width = T) %>% 
  kable_styling(position = "center")
